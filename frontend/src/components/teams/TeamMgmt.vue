    ...
    async addMemberToTeam(): Promise<void> {
      try {
        const { data } = await Send.postRequest<TeamUser>(
          `/teams/${this.teamMembersStore.currentTeamId}/users/${this.selectedUserName.id}`
        );
        ...
        this.snackbar = true;
        this.snackbarText = `User ${ response.data.data.username } was successfully added to ${ this.teamMembersStore.currentTeamName }`;
      } catch (err) {
        this.snackbarColor = "red";
        this.snackbar = true;
        this.snackbarText = `Could not add ${ this.selectedUserName.username } to ${ this.teamMembersStore.currentTeamName }`;
      }
    },
